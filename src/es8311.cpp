...
esp_err_t es8311_codec_init(void)
{
    ...
    if (es8311_read_reg(es_handle, ES8311_CHD1_REGFD, &id1) != ESP_OK ||
        ...
        es8311_read_reg(es_handle, ES8311_CHVER_REGFF, &ver) != ESP_OK) {
        ESP_LOGE(TAG, "ES8311 not responding on I2C (addr 0x%02x)", ES8311_ADDRRES_0);
        es8311_delete(es_handle);
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "ES8311 chip id=0x%02x%02x ver=0x%02x (expect id=0x8311)", id1, id2, ver);
    if (id1 != 0x83 || id2 != 0x11) {
        ESP_LOGE(TAG, "Bad chip ID — aborting (I2C read path or wrong device)");
        es8311_delete(es_handle);
        return ESP_FAIL;
    }

    // Prove write+read before full init
    ESP_RETURN_ON_ERROR(es8311_write_reg(es_handle, ES8311_DAC_REG32, 0x5A), TAG, "volume probe write failed");
    uint8_t probe = 0;
    ESP_RETURN_ON_ERROR(es8311_read_reg(es_handle, ES8311_DAC_REG32, &probe), TAG, "volume probe read failed");
    ESP_LOGI(TAG, "I2C R/W probe REG32 wrote 0x5A read 0x%02x", probe);
    if (probe != 0x5A) {
        ESP_LOGE(TAG, "I2C R/W probe mismatch — aborting");
        es8311_delete(es_handle);
        return ESP_FAIL;
    }
    // --- End diagnostics ---
